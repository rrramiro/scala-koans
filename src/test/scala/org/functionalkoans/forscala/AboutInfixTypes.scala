...
class AboutInfixTypes extends KoanFunSuite with Matchers {
  ...
  koan("""We can make a type infix, meaning that the type can be displayed
         |  in complement between two types in order to make
         |  a readable delaration""") {
  ...
  koan("""Of course we can make this a bit more elegant by creating
         | an infix operator method to use with our infix type""") {
