...
class AboutByNameParameter extends KoanFunSuite with Matchers {
  ...
  koan(
    """() => Int is a Function type that takes a Unit type. Unit is known as
  ...
  koan(
    """A by-name parameter does the same thing as a previous koan but there
  ...
  koan(
    """By name parameters can also be used with an Object and apply to make
